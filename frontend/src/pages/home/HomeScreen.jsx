...
const HomeScreen = () => {
  ...
  const title = trendingContent?.title || trendingContent?.name || "Featured";
  const year =
    trendingContent?.release_date?.split("-")[0] ||
    trendingContent?.first_air_date?.split("-")[0] ||
    "";
  const maturity = trendingContent?.adult ? "18+" : "PG-13";
  const backdrop = trendingContent?.backdrop_path
    ? ORIGINAL_IMG_BASE_URL + trendingContent.backdrop_path
    : null;
  const overview = trendingContent?.overview || "";
  ...
  return (
    <>
      <div className="relative h-screen text-white ">
        ...
        {/* shimmer overlay while hero loads */}
        {imgLoading && (
        ...
        <div className="absolute top-0 left-0 w-full h-full flex flex-col justify-center px-8 md:px-16 lg:px-32">
          ...
          <div className="flex mt-8">
            ...
            {/* FIXED: use trendingContent?.id (not item.id) */}
            <Link
              to={`/watch/${contentType}/${trendingContent?.id}`}
